...
4. :heavy_check_mark: 目前流行的软件生命周期模型有哪些？分别举例说明
...
5. :heavy_check_mark: 软件测试的目的是什么？你认为应该怎么进行软件测试？
...
6. :heavy_check_mark: 什么是软件危机？软件危机的主要表现是什么？怎样消除软件危机？
...
9. :heavy_check_mark: 你认为对于初学者来说，最合适的软件开发模型是哪个？【直接瀑布模型】
10. :heavy_check_mark: 软件开发中的成本有哪些？尝试举例说明
...
:star: 软件工程三要素：软件工程**方法**，软件**工具**，软件工程**过程**
...
:star:【P37】可行性分析包括：**技术**可行性分析、**经济**可行性分析、**社会**可行性分析、**开发方案**可行性分析和**运行**可行性分析，主要是前三项
...
:star: 【P61】软件需求分析的特点：（1）指标确定难（2）需求动态性（3）交流共识难（4）完备一致难（5）深入完善难
...
3. 数据字典。组成：数据流，数据项，文件（文件存储）及基本加工

4. 结构化过程描述（加工过程）。描述工具：**结构化语言，判定树，判定表** 
...
:rocket: 总体设计的总原则：由宏观到微观，逐步求精 :rocket:

软件设计基本原则： 模块化，抽象，内聚和耦合，子系统及模块划分，信息隐蔽等

> :bangbang: 内聚：一个模块内部各个组成元素之间相互结合的紧密程度
>
> :bangbang: 耦合：软件模块之间互相依赖的方式和程度
...
详细设计使用的结构化程序设计是一种设计程序技术，采用**自顶向下、 逐步求精**的设计方法和**单入口单出口**的控制结构。

【P105】详细设计（各模块的设计）采用的工具：程序流程图（单口入，单口出的控制结构），结构流程图，问题分析图
...
（1）OOP开发策略。（先将业务对象转化为数据表，在数据库中创建物理表，然后编写测试业务对象的程序）

（2）自顶向下策略（从顶层模块开始，逐步向下层模块延伸，直到最后编写最底层模块）

（3）自底向上策略（从数据层逐步向业务层和界面层过度）
